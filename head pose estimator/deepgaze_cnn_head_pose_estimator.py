...
class CnnHeadPoseEstimator:
    ...
    def _allocate_yaw_variables(self):
        ...
        def model(data):
            ...
            # Convolution Layer 1
            conv1 = tf.tanh(tf.nn.bias_add(tf.nn.conv2d(X, self.hy_conv1_weights, strides=[1, 1, 1, 1], padding='SAME'),
            ...
            # Convolution Layer 2
            conv2 = tf.tanh(
                tf.nn.bias_add(tf.nn.conv2d(norm1, self.hy_conv2_weights, strides=[1, 1, 1, 1], padding='SAME'),
            ...
            # Convolution Layer 3
            conv3 = tf.tanh(
                tf.nn.bias_add(tf.nn.conv2d(norm2, self.hy_conv3_weights, strides=[1, 1, 1, 1], padding='SAME'),
    ...
    def load_yaw_variables(self, YawFilePath):
        ...
        if (os.path.isfile(YawFilePath) == False): raise ValueError(
    ...
    def _allocate_pitch_variables(self):
        ...
        def model(data):
            ...
            # Convolution Layer 1
            conv1 = tf.tanh(tf.nn.bias_add(tf.nn.conv2d(X, self.hp_conv1_weights, strides=[1, 1, 1, 1], padding='SAME'),
            ...
            # Convolution Layer 2
            conv2 = tf.tanh(
                tf.nn.bias_add(tf.nn.conv2d(norm1, self.hp_conv2_weights, strides=[1, 1, 1, 1], padding='SAME'),
            ...
            # Convolution Layer 3
            conv3 = tf.tanh(
                tf.nn.bias_add(tf.nn.conv2d(norm2, self.hp_conv3_weights, strides=[1, 1, 1, 1], padding='SAME'),
    ...
    def _allocate_roll_variables(self):
        ...
        def model(data):
            ...
            # Convolution Layer 2
            conv2 = tf.tanh(
                tf.nn.bias_add(tf.nn.conv2d(norm1, self.hr_conv2_weights, strides=[1, 1, 1, 1], padding='SAME'),
            ...
            # Convolution Layer 3
            conv3 = tf.tanh(
                tf.nn.bias_add(tf.nn.conv2d(norm2, self.hr_conv3_weights, strides=[1, 1, 1, 1], padding='SAME'),
    ...
    def load_pitch_variables(self, pitchFilePath):
        ...
        if (os.path.isfile(pitchFilePath) == False): raise ValueError(
    ...
    def load_roll_variables(self, rollFilePath):
        ...
        if (os.path.isfile(rollFilePath) == False): raise ValueError(
...
class PnpHeadPoseEstimator:
    ...
    def __init__(self, cam_w, cam_h, dlib_shape_predictor_file_path):
        ...
        """
        if (IS_DLIB_INSTALLED == False): raise ValueError(
        ...
        if (os.path.isfile(dlib_shape_predictor_file_path) == False): raise ValueError(
    ...
    def return_roll_pitch_yaw(self, image, radians=False):
        ...
        landmarks_2D = self._return_landmarks(inputImg=image, roiX=0, roiY=img_w, roiW=img_w, roiH=img_h,
